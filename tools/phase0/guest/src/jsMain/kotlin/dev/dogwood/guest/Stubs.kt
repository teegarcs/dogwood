...
fun Row(
  ...
  ComposeNode<WidgetNode, DogwoodApplier>(
    ...
    update = {
      set(modifier) { if (it.elements.isNotEmpty()) recording.recorder.modifiers(id, it.elements) }
      // Phase 0 stand-in for `Modifier.clickable`, which carries a lambda argument and so
      // needs the modifier subsystem Phase 2 defines. Registering the slot here keeps the
      // reference screen honest about its "one event handler per row" without inventing a
      // modifier grammar ahead of its Architecture Decision Record (ADR).
      set(onClick) { handler ->
