...
The host application does not learn about your new screen, and it does not need to. It was built already knowing essentially the whole Compose Application Programming Interface (API), so anything you assemble out of Compose is something it can already draw.
...
**1. Some of Compose is unavailable, and some arrives later than the rest.** Measured across ten modules and 450 composables ([`tools/measure-compose-surface.py`](tools/measure-compose-surface.py)):

- **6.0% are structurally unreachable.** Anything whose lambda the rendering engine invokes inside a frame — `Canvas`, `Modifier.drawBehind`, `Modifier.pointerInput` — plus custom `Layout` and `SubcomposeLayout`.
- **12.9% need a hand-written protocol** because they take a live state holder you read or call: `LazyListState`, `SnackbarHostState`, `FocusRequester`, `TextFieldState`. These arrive one subsystem at a time.
- **`LazyColumn` and text fields are in those groups.** Both are planned as bespoke subsystems with Dogwood-specific shapes — a lazy list takes a `placeholder` that Compose's has no equivalent for, and a text field takes a state object rather than a plain `String`.
- **81.1% are generated**, once the `Modifier` subsystem lands.

Note that function coverage overstates parameter coverage: a composable may be available while one of its parameters — commonly `interactionSource` — is not yet passable.
...
**6. Target the oldest client you support.** Adoption curves are long. The dictionary check enforces this, but design for it — capability branching is available when you need it:

```kotlin
if (DogwoodVersion >= 7u) {
  ModernThing()
} else {
  FallbackThing()
}
...
- **Rendering is fully native.** Layout, drawing, animation, scrolling, VoiceOver, TalkBack, and the keyboard are all real Compose Multiplatform. Your screen is not a canvas or a web view; it is the same widget tree a statically compiled screen produces, which is why accessibility works without you doing anything.

---

## 7. One Screen, Three Platforms

You write a screen once. It runs on Android, iOS, and Web, because the host renders it with Compose Multiplatform and Compose Multiplatform runs on all three.
...
|---|---|---|
| New component available to you | After a client release | Immediately, if it exists in Compose |
...
4. **The closest prior art, Cash App's Redwood, is no longer under active development,** and no public rationale exists. Understanding why is the single highest-value open question.
5. **The bespoke subsystems are the larger half of the work.** Redwood needed ten modules for lazy lists alone. The generated bindings are the part that scales; the hand-written subsystems are the part that takes the time.
