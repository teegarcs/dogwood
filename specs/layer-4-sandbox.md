...
**What the boundary actually costs, measured.** Phase 0.3 has now run ([ADR-006](../adrs/layer-4/ADR-006-batch-crossing-is-guest-encoding.md)). For the reference screen's initial batch — 572 changes, 19,795 bytes — on a development machine faster than the gate device: the whole crossing is **24.06 ms** at p50, of which **guest-side encoding is 99.4%** and `CallChannel` transport is **1.0%**. Cost is linear in bytes at roughly 1.2 microseconds per byte. Two consequences for anyone implementing this layer. First, **never hand `sendChanges` a string you encoded yourself**: Zipline encodes through `encodeToDynamic` plus QuickJS's native `JSON.stringify`, which is 47% faster than `kotlinx.serialization`'s pure-Kotlin encoder for identical bytes, and passing a `String` argument forfeits that. Second, steady state is cheap — a one-change batch crosses in 0.144 ms — so the expensive crossing is the *initial* batch, once per screen open, not the per-frame diff.
