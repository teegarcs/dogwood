...
**Guest failure must not tear the host tree, and it must be attributable.** Three rules. First, a `ChangeBatch` is emitted only after a composition pass completes — an exception mid-composition produces telemetry, never a partial batch, so the host tree is always a consistent frame. Second, on `handleUncaughtException` the host's recovery path is declared, not improvised: report with the payload version and manifest identifiers, tear down the experience, and either reload it or show the host's fallback surface. Third, **symbolication is built, and it works differently on the two profiles**. This was written when it was an open item and guest stack traces were line noise; it is not one now. On the Zipline profile, Zipline applies source maps *at build time*, so what reaches `handleUncaughtException` already names real Kotlin files from the payload with nothing to deploy alongside it — attribution is file-level, because function names stay minified and line numbers do not survive the size-optimized build ([ADR-059](../adrs/layer-5/ADR-059-a-guest-crash-a-host-can-read.md)). On the Web profile the frames cross with the failure and carry exact line and column offsets under minified names, and translation is an **offline** step against a source map the build keeps and deliberately does not serve — `tools/symbolicate/resolve.py` ([ADR-063](../adrs/layer-5/ADR-063-a-web-crash-carries-its-frames.md)). So the requirement this paragraph placed on Layer 2 stands and is met: **retain the mapping artifacts per build**, because a pipeline that discards them gets frames it cannot resolve. What has changed is that the translation step is a tool that exists rather than work that has not been done.

The recovery path deserves one correction from the same measurement. `handleUncaughtException` is called on the host **inside a Zipline service dispatch**, which means a handler that throws has its throw returned to the *guest* as the call's failure rather than reaching the host process — so "tear down the experience" must be something the handler *does*, never something it signals by rethrowing.
