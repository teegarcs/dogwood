...
### Who Decides an Environment-Dependent Value

One question answers it: **who owns what this value means?**

- **The design system owns it** (brand colour, type ramp, icon): name it — `Color.token("primary")`, `style = "titleLarge"`, `Icon(name = "flight")`. The name is a versioned contract with the client, unknown names degrade and are reported, and a theme change costs no traffic.
- **This screen owns it** (a one-off tint, a chart palette): decide it yourself. The host injects its environment — `isSystemInDarkTheme()`, `LocalHostEnvironment` — so ordinary Kotlin picks a literal (`Color(0xFF…)`) and the wire carries the finished answer. No contract, at the cost of one small message when the environment changes.

There is no third path. Logic does not cross the wire; only names and results do ([ADR-025](../adrs/layer-5/ADR-025-where-a-colour-decision-lives.md)).
...
The checker carries two further **hard rejection obligations**, added after adversarial review ([Layer 5 ADR-005](../adrs/layer-5/ADR-005-corrected-coverage-and-bespoke-subsystem-list.md)). **Both are now enforced** by `io.github.teegarcs.dogwood.guest` ([ADR-050](../adrs/layer-5/ADR-050-the-authoring-check.md)), which joins `check` and names the replacement for every rejection:
