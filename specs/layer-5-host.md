...
**Plurals split the same line the other way**: the host picks the Unicode category, because that is locale data the sandbox has no table for, and the payload supplies the words. `#` is replaced by the *formatted* count. On a desktop Java Virtual Machine the category falls back to the English rule, which is wrong for most languages and is said out loud rather than left to be discovered.
