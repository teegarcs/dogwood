...
- Maintains one built artifact per supported client binding-dictionary version.
...
* **JavaScript modules:** One JavaScript file per Kotlin module, retaining module structure so the loader can cache and reuse unchanged modules across releases.
...
* **Binding dictionary artifacts:** One per shipped client version, published by the client build. These are inputs, not outputs, of this layer.
...
4. **Milestone 4 — Per-version build matrix.** Take two synthetic dictionary versions and prove the pipeline emits a correct, distinct artifact for each.
