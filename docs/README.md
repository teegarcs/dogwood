...
layer specifications, sixty architecture decision records and a conformance catalogue, with no
...
- **The ADRs are the "why".** Every maintenance-phase decision has one, each lists the documents it
  changed, and the index summarises all sixty. When a line of code seems wrong, its ADR usually
