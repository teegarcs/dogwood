...
- **The bindability rule applies to registered signatures unchanged**: parameters must be primitives, value classes, deferred-expressible objects, composition-time slots, or discrete events. The build fails registration for a signature that violates it, with an error naming the offending parameter. **That sentence was not true until 2026-09-13**: for the thirteen days between the first product surface and [ADR-068](ADR-068-the-generator-refuses-what-it-cannot-bind.md), a rejected component was one line on standard output, an exit code of zero and a green build, and Umbra's stepper sat unbindable for as long as it existed. It is true now, and enumerations — the most common shape the rule had no answer for — are declared on the surface. Curated design-system signatures satisfy this far more often than raw Material does — which is precisely why registration is high-leverage.
