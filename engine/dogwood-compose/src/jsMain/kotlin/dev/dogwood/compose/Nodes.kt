...
/**
 * The event tag chain element *n*'s callback is reported under.
 *
 ...
 * different problem.
 *
 * It was `ANIMATION_EVENT_BASE` while an animation's completion was the only thing a modifier
 * element could report. `clickable` reports through the same slot, and the rename says so: the
 * mechanism is "one event per element that wants one", and what the element does with it is the
 * element's business.
 */
const val ELEMENT_EVENT_BASE = 1000
