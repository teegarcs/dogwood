...
* **Compose Multiplatform renders natively:** Layout, measure, draw, accessibility, and text input all run natively at full speed. **Which renderer that is differs by platform, and only one of them costs anything.** On Android, Compose draws through the platform's own `Canvas` and graphics stack — the rendered tree is identical to a statically compiled screen, and nothing is linked or downloaded. On iOS and desktop it is Skiko over Metal and AWT respectively, linked into the binary and paid for once at install. On Web it is Skiko compiled to WebAssembly and **fetched over the network before the first frame** — 8.24 MB of it, which is why Web is the only target where the renderer is a delivery problem ([ADR-030](adrs/layer-5/ADR-030-web-page-weight-measured.md)) — a problem measured end to end at **17.4 seconds to first frame on a Fast-3G connection** against 135 milliseconds unthrottled, transfer outweighing every other cost by about a hundred to one ([ADR-038](adrs/layer-5/ADR-038-first-frame-is-transfer-bound.md)). See [Layer 5 ADR-001](adrs/layer-5/ADR-001-host-native-compose-owns-semantics-and-input.md) and [ADR-004](adrs/layer-5/ADR-004-compose-multiplatform-sole-host-target.md).
...
3. Parameters that govern safety or affordance — `enabled`, `checked`, `readOnly`, `selected` — **must** be marked in the dictionary, and a widget that owns one is **withheld** rather than drawn when it arrives carrying a property this client cannot read. Implemented in [ADR-031](adrs/layer-5/ADR-031-safety-relevant-parameters.md), which corrects two things this sentence originally got wrong.

   **The decision is made per widget, not per property**, because of what a client can actually observe. The dangerous case is a newer payload on an older client: the payload sends a property tag this client's dictionary does not contain, so the binding never reads it and draws a normal control. The client cannot ask "was that the one that disables this?" — the tag comes from a dictionary it has never seen. What it *can* observe is that something unreadable arrived on this node, which is a whole-node fact, so the policy is a whole-node policy: a widget that owns an affordance refuses to draw when anything on it is unreadable, because one of those things might be the affordance.

   **The marking is what a parameter governs, not what it is called.** `Badge` carries `selected` and is deliberately unmarked, because a badge is read rather than operated and getting it wrong costs appearance; `Chip` carries the same name bound to a handler and is marked. Withholding every widget over cosmetic skew would turn a slightly wrong screen into a screen with holes in it, which is a worse outcome than the skew — this rule is narrow on purpose, and it is the **only** place in this system where a client stops drawing rather than degrading.
